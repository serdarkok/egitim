<template>
  <div>
    <el-table :data="tableData" style="width: 100%">
      <el-table-column fixed prop="createdAt" filter-method="changeDate(createdAt)" label="Tarih"></el-table-column>
...
export default {
  ...
  async asyncData(context) {
    ...
    computed : {
        reversedMessage: function (context) {
            // `this` points to the vm instance
            // return this.tableData[0].createdAt;
            return context.$moment(this.tableData[0].createdAt).format('DD MMMM YYYY')
            }
    },

    methods : {
        changeDate(veri) {
          return this.$moment(veri).format('DD MMMM YYYY')  
        }
    }
};
</script>
